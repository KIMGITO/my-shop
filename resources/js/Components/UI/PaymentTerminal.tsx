// resources/js/Pages/POS/PaymentPage.tsx
...
export default function PaymentPage({
    ...
    useEffect(() => {
        if (activeTab === "mpesa") {
            ...
            // Per request: wait for manual entry (default to 0)
            setMpesaAmount(0);
            setCashAmount(0);
            setCreditAmount(0);
        }
    ...
    useEffect(() => {
        if (activeTab === "split") {
            ...
            if (isOnlyCash) setActiveTab("cash");
            if (isOnlyMpesa) setActiveTab("mpesa");
            if (isOnlyCredit) setActiveTab("credit");
        }
    }, [cashAmount, mpesaAmount, creditAmount, total]);
    // Removed activeTab from dependencies to prevent the "snap-back" loop
    ...
    const handleSplitChange = (
        ...
        if (type === "mpesa") {
            // If in split mode, enforce the cap. If in dedicated tab, allow full entry
            if (activeTab === "split") {
    ...
    const canComplete =
        paidTotal >= total && (creditAmount > 0 ? !!selectedCustomerId : true);
                                    ...
                                    {activeTab === "split" ? (
                                        <div className="space-y-4 md:space-y-6">
                                            ...
                                            <PaymentInput
                                                ...
                                                error={!selectedCustomerId ? 'Select Customer for credit' : ''}
                                            />
                                        </div>
                                    ) : (
                                        <PaymentInput
...
function PaymentInput({ label, value, onChange, disabled, error, size = "lg" }: any) {
    return (
        <div
            className={cn(
                "space-y-2 w-full",
